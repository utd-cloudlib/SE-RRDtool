...
// <<<<<<<<  modified by Shuai
    ...
    while (42) {
        ...
        opt = getopt_long(argc, s_argv, "rf", long_options, &option_index);
    ...
    if ((argc - optind) != 2) {
        rrd_set_error("usage rrdtool %s [--range-check/-r] "
                      "[--force-overwrite/-f] file.xml file.rrd", s_argv[0]);
        return (-1);
    }

    old_locale = setlocale(LC_NUMERIC, "C");

    rrd = parse_file(s_argv[optind]);
    ...
    if (write_file(s_argv[optind + 1], rrd) != 0) {
        local_rrd_free(rrd);
        return (-1);
    }
    local_rrd_free(rrd);
