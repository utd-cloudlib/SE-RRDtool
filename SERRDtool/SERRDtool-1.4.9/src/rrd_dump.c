...
// <<<<<<<<  modified by Shuai
    ...
    while (42) {/* ha ha */
        ...
        opt = getopt_long(argc, s_argv, "d:h:n", long_options, &option_index);
    ...
    rc = rrdc_flush_if_daemon(opt_daemon, s_argv[optind]);
    if (opt_daemon) free(opt_daemon);
    if (rc) return (rc);

    if ((argc - optind) == 2) {
        rc = rrd_dump_opt_r(s_argv[optind], s_argv[optind + 1], opt_header);
    } else {
        rc = rrd_dump_opt_r(s_argv[optind], NULL, opt_header);
    }
    arg_free(s_argv);
