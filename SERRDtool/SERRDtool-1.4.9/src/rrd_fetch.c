...
// <<<<<<<<  modified by Shuai
    char **s_argv = redirect(argc, argv, optind);
    if(s_argv != NULL) { // SE mode
	if(strcmp(argv[1], "-s") == 0) { // remove "-se", unitName, "-un", unit, "-et", entityType, "-en", entity
	    argc = argc - 8;
	}
...
printf("%d-th s_argv: %s: \t%d. \n", i, s_argv[i], strlen(s_argv[i]));
    ...
    while (1) {
        ...
        opt = getopt_long(argc, s_argv, "r:s:e:d:", long_options, &option_index);
    ...
    *step = step_tmp;
    ...
    if (optind + 1 >= argc) {
        rrd_set_error("Usage: rrdtool %s <file> <CF> [options]", s_argv[0]);
        return -1;
    }

    status = rrdc_flush_if_daemon(opt_daemon, s_argv[optind]);
    if (opt_daemon) free (opt_daemon);
    if (status) return (-1);

    cf = s_argv[optind + 1];

    status = rrd_fetch_r(s_argv[optind], cf, start, end, step,
            ds_cnt, ds_namv, data);
    arg_free(s_argv);
