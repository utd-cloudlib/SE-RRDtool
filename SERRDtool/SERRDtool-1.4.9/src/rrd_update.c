...
// <<<<<<<<  modified by Shuai
    ...
    while (1) {
        opt = getopt_long(argc, s_argv, "t:d:", long_options, &option_index);
    ...
    {
        rc = rrd_update_r(s_argv[optind], tmplt,
                      argc - optind - 1, (const char **) (s_argv + optind + 1));

    }
    else /* we are connected */
    {
        rc = rrdc_update (s_argv[optind], /* file */
                          argc - optind - 1, /* values_num */
                          (const char *const *) (s_argv + optind + 1)); /* values */
