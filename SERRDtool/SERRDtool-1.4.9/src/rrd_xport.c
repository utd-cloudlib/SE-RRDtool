...
// ---- test------
    FILE *fp;
    fp = fopen("xport_arg.txt", "w");
    int itr;
    for(itr = 0; itr < argc; ++itr) {
	printf("%d-th s_argv from xport: %s \n", itr, s_argv[itr]);
    }
    fclose(fp);
// ---- test------
...
while (1) {
        ...
        opt = getopt_long(argc, s_argv, "s:e:m:d:", long_options, &option_index);
        ...
        case '?':
            rrd_set_error("unknown option '%s'", s_argv[optind - 1]);
            return -1;
        }
    }
    ...
    rrd_graph_script(argc, s_argv, &im, 0);
    if (rrd_test_error()) {
        im_free(&im);
        return -1;
    }
