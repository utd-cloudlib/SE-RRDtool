...
// <<<<<<<<  modified by Shuai
	...
	char** xport_argv;
	int* offset;
	int ind = 0, mem_size = 0, length;
	char rrd_filename[256];
	...
	fp = fopen("xport.txt", "r");
	if (fp == NULL)
            exit(EXIT_FAILURE);
printf("Parsing xport\n");
	if((read = getline(&line, &len, fp)) != -1) {
	    xport_argc = atoi(line);
	}
printf("Argc parsed. \n");
	if((read = getline(&line, &len, fp)) != -1) {
	    strcpy(rrd_filename, line);
	}
printf("Rrd filename parse.\n");
	// calculate memory 
	offset = (int*)malloc(sizeof(int) * xport_argc);
  	if(offset == NULL) {
	    // error processing while malloc
	    rrd_set_error("allocating unknown");
	    return -1;
    	}
	xport_argv = (char**)malloc(sizeof(char*) * xport_argc);
  	if(xport_argv == NULL) {
	    // error processing while malloc
	    rrd_set_error("allocating unknown");
	    return -1;
	...
	while ((read = getline(&line, &len, fp)) != -1) {
	    ...
	    if(len != 0) {
		length = strlen(line);
		//strcpy(xport_argv[ind], line);
		offset[ind] = length;
		mem_size += length + 1;
printf("Calculating memory:%d \t %d \t %d \t %s\n", length + 1, strlen(line), mem_size, line);
		ind++;
	    }
	}
...
printf("Argv parse.\n");
	...
	// allocate memory
	xport_argv[0] = (char*)malloc(sizeof(char) * mem_size);
  	if(xport_argv[0] == NULL) {
	    // error processing while malloc
	    rrd_set_error("allocating unknown");
	    return -1;
    	}

	for(ind = 1; ind < xport_argc; ++ind) {
	    xport_argv[ind] = xport_argv[ind - 1] + offset[ind - 1];
	}
printf("Memory allocated.\n");
	...
	if (fp == NULL)
            exit(EXIT_FAILURE);
printf("Start updating 1.\n");
	if((read = getline(&line, &len, fp)) != -1) {
	    //parse argc
	}
printf("Start updating 2.\n");
	if((read = getline(&line, &len, fp)) != -1) {
	    //parse rrd filename
	}
printf("Start updating 3.\n");
	ind = 0;
	while ((read = getline(&line, &len, fp)) != -1) {
	    len = strlen(line);
