...
//#include "rrd.h" // modified by Shuai
//#include "rrd_xport.h" // modified by Shuai
//#include <locale.h> //modified by Shuai
...
extern int       rrd_xport(
    ...
    unsigned long *,
    char ***,
    rrd_value_t **); //modified by Shuai
*/
int rrd_create(
    int argc,
    char **argv)
{
    printf("rrd_create Entered. \n");
...
// <<<<<<<<<<< modified by Shuai
    char **s_argv = redirect(argc, argv, optind);   // changed by Shuai

    int len, ind, indv;

    if(s_argv != NULL) { // SE mode
	if(strcmp(argv[1], "-se") == 0) { // remove "-se", unitName, "-un", unit, "-et", entityType, "-en", entity
	    argc = argc - 7; // and add quantitativeDefinition at the beginning
	}
	else if (strcmp(argv[1], "-sh") == 0) { // remove "-sh"
	    argc = argc - 1;
	}


    while (1) {
        opt = getopt_long(argc - 1, &s_argv[1], "Ob:s:", long_options, &option_index);
printf("opt parsed: %c\n", opt);
        if (opt == EOF)
            break;
        ...
        case '?':
            if (optopt != 0)
                ...
                rrd_set_error("unknown option '%s'", s_argv[optind]);
            return (-1);
        }
    }
    if (optind + 1 == argc) {
...
int i;
for(i = 0; i < argc; ++i) {
printf("%d-th s_argv: %s: \t%d. \n", i, s_argv[i], strlen(s_argv[i]));
}
	if(strlen(s_argv[0]) != 0) { // Reasoning result with quantitativeDefinition
	    // xport
	    char **xport_argv;
	    char cdef[4096];
	    char def[256];
	    ...
	    int mark = 0;
	    int loc = 0, len = strlen(s_argv[optind + 1]);
	    char* xport_definition = s_argv[0];
	    int xport_argc = 2, xport_index = 0;

	    for(ind = 0; ind < len; ++ind) {
		if(s_argv[optind + 1][ind] == '/') loc = ind;
	    }
printf("Path: %d \t %s \t file: %d \t %s \n", len, s_argv[optind + 1], loc + 1, hname);
	    strcpy(hname, &s_argv[optind + 1][loc+1]);
printf("Path: %d \t %s \t file: %d \t %s \n", len, s_argv[optind + 1], loc + 1, hname);
	    len = strlen(xport_definition);
	    strcpy(cdef, "CDEF:");
	    strcat(cdef, hname); // hanlde as name
	    strcat(cdef, "=");
	    strcpy(def, "DEF:");
	    strcpy(dname, "");
	    
	    for(ind = 0, indv = 0; ind <= len; ++ind, ++indv) {
		if(xport_definition[ind] == ',') {
		    if(mark) { 
			xport_argc++;
		    }
		    mark = 0;
		}
		else if(xport_definition[ind] == ':') { // potential for dname
		    if(!mark){
			mark = 1;
		    }
		}
		else if(xport_definition[ind] == '\0') {
		    if(mark) {
			xport_argc++;
		    }
		}
	    }
	    xport_argc = xport_argc + 4;

	    if((xport_argv = (char**)malloc(sizeof(char*) * xport_argc)) == NULL){
		rrd_set_error("allocating unknown");
		return (-1);
	    }

	    if((xport_argv[xport_index] = (char*)malloc(sizeof(char) * (strlen("xport") + 1))) == NULL){
		rrd_set_error("allocating unknown");
		return (-1);
	    }
	    strcpy(xport_argv[xport_index++], "xport");
	    if((xport_argv[xport_index] = (char*)malloc(sizeof(char) * (strlen("-se") + 1))) == NULL){
		rrd_set_error("allocating unknown");
		return (-1);
	    }
	    strcpy(xport_argv[xport_index++], "-se");


	    for(ind = 0, indv = 0; ind <= len; ++ind, ++indv) {
		if(xport_definition[ind] == ',') { // new item needs to be processed
		    item[indv] = '\0';
printf("when , item: %s \t %d \n", item, mark);
		    if(mark) { // ready for a DEF
			strcat(def, item);
printf("when 1 def: %s \t %d \n", def, mark);
	    		if((xport_argv[xport_index] = (char*)malloc(sizeof(char) * (strlen(def) + 1))) == NULL)			{
			    rrd_set_error("allocating unknown");
			    return (-1);
	    		}
			strcpy(xport_argv[xport_index++], def);
printf("when 1 xport_argv[xport_index++]: %d \t %d \t %s \n", xport_argc,xport_index, xport_argv[xport_index - 1]);
			strcpy(def, "DEF:");
printf("when 1 def: %s \t %d \n", def, mark);
		    }
		    else { // directly into CDEF
			strcat(cdef, item);
			strcat(cdef, ",");
		    }
...
printf("when , def: %s\n", def);
			...
			strcat(cdef, dname);
			strcat(cdef, ",");
printf("when : item: %d \t % d %s\n", indv, strlen(item), item);
			item[indv] = ':';
		    }
		    else {
			if(item[indv - 1] == ':') {
...
printf("when 0 item: %s\n", item);
		    ...
		    item[indv] = xport_definition[ind];
		}
	    }
	   
	    if((xport_argv[xport_index] = (char*)malloc(sizeof(char) * (strlen(cdef) + 1))) == NULL){
		rrd_set_error("allocating unknown");
		return (-1);
	    }
	    strcpy(xport_argv[xport_index++], cdef);
	    ...
	    strcpy(outfile, s_argv[optind + 1]);
	    len = strlen(outfile);
	    outfile[len - 3] = 'x';
	    outfile[len - 2] = 'm';
	    outfile[len - 1] = 'l';
	    if((xport_argv[xport_index] = (char*)malloc(sizeof(char) * (strlen(">") + 1))) == NULL){
		rrd_set_error("allocating unknown");
		return (-1);
	    }
	    strcpy(xport_argv[xport_index++], ">");
	    if((xport_argv[xport_index] = (char*)malloc(sizeof(char) * (strlen(outfile) + 1))) == NULL){
		rrd_set_error("allocating unknown");
		return (-1);
	    }
	    strcpy(xport_argv[xport_index++], outfile);

for(ind = 0; ind < xport_argc; ++ind) {
  printf("%d-th xport_argv: %s \n", ind, xport_argv[ind]);
}
	    int xxsize;
	    time_t start, end;
...
//#ifdef HAVE_RRD_GRAPH
	    //rrd_xport(xport_argc, xport_argv, &xxsize, &start, &end, &step, &col_cnt, &legend_v, &data);
//#endif

	    FILE *fp;
	    fp = fopen("xport.txt", "w");
	    fprintf(fp, "%d\n", xport_argc);
	    fprintf(fp, "%s\n", s_argv[optind + 1]); //rrd_filename
	    for(ind = 0; ind < xport_argc; ind++) {
		fprintf(fp, "%s\n",xport_argv[ind]);
	    }
	    fclose(fp);

	    for(ind = xport_argc - 1; ind >= 0 ; --ind) {
printf("Free xport[ind]: %d \n", ind);
		free(xport_argv[ind]);
	    }
printf("Free xport.\n");
	    free(xport_argv);
...
printf("Ready to create.\n");
	rc = rrd_create_r(s_argv[optind + 1],
                      pdp_step, last_up,
                      argc - optind - 2, (const char **) (s_argv + optind + 2));  // changed by Shuai
printf("Finish to create.\n");
	arg_free(s_argv);
printf("Exit create.\n");
	return rc;

    } // end if s_argv not null
// >>>>>>>>>>> modified by Shuai
