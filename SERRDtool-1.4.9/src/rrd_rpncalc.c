...
struct curl_reasoning_st getFileName(int argc, char **argv, int opt);
...
struct curl_reasoning_st json_parse(json_object * jobj) {
	...
	if(strcmp(key, "handle") == 0) {
	    strcpy(res.handle, json_object_get_string(val));
...
struct curl_reasoning_st getFileName(int argc, char **argv, int opt) {
    ...
    // url to test site
    char *url = "http://localhost:8080/SERRDtool/reasoning/WebService/metas?metricName=abcdefghijklmnh&DSName=abcdefgh&unitName=percent&unit=18&entityType=host&entity=host0001&startTime=15&endTime=18";

    FILE * fp;

    fp = fopen("getFileName.txt", "a");
    int i = 0;
    for(i = 0; i < argc; ++i) {
        fprintf(fp, "%s\n", argv[i]);
    }
...
char** redirect(int argc, char** argv, int optind) {
    ...
    int ind, loc = -1, i, loc_DS = 0, j, mem_size = 0, len = 0;
    int offset[argc];
    char c;
	...
	struct curl_reasoning_st res = getFileName(argc, argv, optind);
